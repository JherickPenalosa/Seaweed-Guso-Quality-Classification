...
class ImageUploadArea extends StatefulWidget {
  final ValueChanged<SelectedImage?> onImageChanged;
...
class _ImageUploadAreaState extends State<ImageUploadArea> {
  ...
  Future<void> _pickImage() async {
    ...
    if (!selectedImage.isSupportedFormat) {
      return;
    }
        ...
        if (hasImage) ...[
          Row(
            children: [
              ...
              TextButton.icon(
                onPressed:
                    widget.isAnalyzing ? null : _pickImage,
                icon: const Icon(
                  Icons.swap_horiz,
                  size: 18,
                ),
              ...
              TextButton.icon(
                onPressed:
                    widget.isAnalyzing ? null : _removeImage,
                icon: const Icon(
                  Icons.close,
                  size: 18,
                ),
  ...
  Widget _buildEmptyState() {
    return InkWell(
      onTap: widget.isAnalyzing ? null : _pickImage,
      child: const Center(
        child: Column(
